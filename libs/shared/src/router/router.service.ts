...
export class RouterService {
  ...
  private populateEventMap() {
    this.events[RouteInterceptorEvents.NavigationStart] = this.router.events.filter(
      (event) => event instanceof NavigationStart
    );
    this.events[RouteInterceptorEvents.NavigationEnd] = this.router.events.filter(
      (event) => event instanceof NavigationEnd
    );
    this.events[RouteInterceptorEvents.NavigationCancel] = this.router.events.filter(
      (event) => event instanceof NavigationCancel
    );
    this.events[RouteInterceptorEvents.NavigationError] = this.router.events.filter(
      (event) => event instanceof NavigationError
    );
    this.events[RouteInterceptorEvents.ConfigLoadStart] = this.router.events.filter(
      (event) => event instanceof RouteConfigLoadStart
    );
    this.events[RouteInterceptorEvents.ConfigLoadEnd] = this.router.events.filter(
      (event) => event instanceof RouteConfigLoadEnd
    );
    this.events[RouteInterceptorEvents.RouteRecognized] = this.router.events.filter(
