...
import { LayoutStateModel } from '../layout/layout-state.model';
import { sharedSelectors, SharedState } from '../reducers';

@Directive({
  selector: '[sfEditPanel]'
})
export class EditPanelDirective implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    this.store
      .pipe(select(sharedSelectors.getLayoutStatus))
      .takeUntil(this.onDestroy)
      .subscribe((status: LayoutStateModel) => {
        this.widths = status.widths;
        this.setWidths();
        setTimeout(() => {
   ...
   * element with the `className` `sf-content` and `parentElement` should always be it's first
   * child element.
   * @param {*} childElement
   */
  setParentElements(childElement: any) {
    ...
    if (element.className !== 'sf-content') {
